    ...
    class GridItem : VisualElement, IGridItem
    {
        ...
        public void BindWithItem(BaseAssetData assetData)
        {
            ...
            if (m_AssetData == null)
            {
                ...
                m_AssetNameLabel.text = string.Empty;
                m_AssetNameLabel.tooltip = string.Empty;
                return;
            }
        ...
        void Refresh()
        {
            ...
            // Only fetch data if we're online - avoids loading icon when offline with no cached data
        ...
        void OnImportedAssetInfoChanged(AssetChangeArgs assetChangeArgs)
        {
                ...
                || assetChangeArgs.Updated.Any(a => a.AssetId == m_AssetData.Identifier.AssetId))
            {
                ...
                {
                    // Asset was removed from project but still exists as cloud data.
                    // Keep the current data and refresh to update the status icons (e.g. remove checkmark).
                    Refresh();
                }
            }
        }
