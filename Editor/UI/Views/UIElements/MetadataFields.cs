...
namespace Unity.AssetManager.UI.Editor
{
    ...
    {
        readonly MultiValueDropdownField m_DropdownField;

        public SingleSelectionMetadataField(List<Core.Editor.SingleSelectionMetadata> representedMetadata,
            List<string> choices)
            : base(representedMetadata)
        {
            m_DropdownField = new MultiValueDropdownField(representedMetadata.Select(x => x.Value).ToList(),
                choices);
            m_DropdownField.RegisterValueChangedCallback(evt => OnValueChanged());

            Add(m_DropdownField);

            // Ensure the displayed value is applied to metadata even if the user doesn't interact
            var representedValue = representedMetadata.FirstOrDefault()?.Value ?? string.Empty;
            var selectedValue = string.IsNullOrEmpty(representedValue)
                ? choices.FirstOrDefault()
                : representedValue;

            if (!string.IsNullOrEmpty(selectedValue))
            {
                m_DropdownField.SetValueWithoutNotify(selectedValue);
                OnValueChanged();
            }
        }
        ...
        void OnValueChanged()
        {
            m_RepresentedMetadata.ForEach(x => x.Value = m_DropdownField.value);
            InvokeValueChanged();
        }
    }
