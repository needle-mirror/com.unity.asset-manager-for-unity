...
namespace Unity.AssetManager.UI.Editor
{
    static partial class UssStyle
    {
        ...
        public static readonly string ReimportFoldoutWarningContainer = "reimport-window-warning-container";
        public static readonly string ReimportFoldoutWarningIcon = "reimport-window-warning-icon";
    }
    ...
    class ReimportFoldout : Foldout
    {
        ...
        internal ReimportFoldout(AssetDataResolutionInfo assetDataResolutionInfo, bool avoidRollingBackAssetVersion)
        {
            ...
            text = string.Empty;
            value = assetDataResolutionInfo.HasTrackingOverlap;
            ...
            if (assetDataResolutionInfo.HasTrackingOverlap)
            {
                const int k_MaxOverlapLines = 10;
                var warningContainer = new VisualElement();
                warningContainer.AddToClassList(UssStyle.ReimportFoldoutWarningContainer);
                var warningIcon = new Image();
                warningIcon.AddToClassList(UssStyle.ReimportFoldoutWarningIcon);
                warningContainer.Add(warningIcon);
                var overlapHeaderText = L10n.Tr(Constants.ReimportFoldoutTrackingOverlapHeader);
                var overlaps = assetDataResolutionInfo.TrackingOverlaps;
                var lines = overlaps
                    .Take(k_MaxOverlapLines)
                    .Select(o => $" - {o.FilePath} -> {o.ConflictingAssetName} ({o.ConflictingAssetId})");
                var message = overlapHeaderText + "\n" + string.Join("\n", lines);
                if (overlaps.Count > k_MaxOverlapLines)
                    message += $"\n... and {overlaps.Count - k_MaxOverlapLines} more.";
